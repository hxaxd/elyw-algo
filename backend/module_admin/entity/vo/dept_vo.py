...
class DeptModel(BaseModel):
    ...
    parent_id: Optional[int] = Field(default=None, description='父小组id')
    ancestors: Optional[str] = Field(default=None, description='祖级列表')
    dept_name: Optional[str] = Field(default=None, description='小组名称')
    ...
    status: Optional[Literal['0', '1']] = Field(default=None, description='小组状态（0正常 1停用）')
    ...
    @NotBlank(field_name='dept_name', message='小组名称不能为空')
    @Size(field_name='dept_name', min_length=0, max_length=30, message='小组名称长度不能超过30个字符')
...
class DeleteDeptModel(BaseModel):
    ...
    dept_ids: str = Field(default=None, description='需要删除的小组id')
    update_by: Optional[str] = Field(default=None, description='更新者')
