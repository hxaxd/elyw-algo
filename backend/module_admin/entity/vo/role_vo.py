...
class RoleModel(BaseModel):
    ...
    data_scope: Optional[Literal['1', '2', '3', '4', '5']] = Field(
        default=None,
        description='数据范围（1：全部数据权限 2：自定数据权限 3：本小组数据权限 4：本小组及以下数据权限 5：仅本人数据权限）',
    )
    menu_check_strictly: Optional[Union[int, bool]] = Field(default=None, description='菜单树选择项是否关联显示')
    dept_check_strictly: Optional[Union[int, bool]] = Field(default=None, description='小组树选择项是否关联显示')
...
class RoleDeptQueryModel(BaseModel):
    ...
    depts: List = Field(default=[], description='小组信息')
    checked_keys: List[int] = Field(default=[], description='已选择的小组ID信息')
...
class AddRoleModel(RoleModel):
    ...
    dept_ids: List = Field(default=[], description='小组ID信息')
