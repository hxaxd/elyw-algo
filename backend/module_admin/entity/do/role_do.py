...
class SysRole(Base):
    ...
    data_scope = Column(
        String(1),
        default='1',
        comment='数据范围（1：全部数据权限 2：自定数据权限 3：本小组数据权限 4：本小组及以下数据权限）',
    )
    menu_check_strictly = Column(Integer, default=1, comment='菜单树选择项是否关联显示')
    dept_check_strictly = Column(Integer, default=1, comment='小组树选择项是否关联显示')
...
class SysRoleDept(Base):
    ...
    dept_id = Column(Integer, primary_key=True, nullable=False, comment='小组ID')
